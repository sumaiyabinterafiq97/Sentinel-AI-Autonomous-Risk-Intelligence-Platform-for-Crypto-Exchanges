...
import com.sentinel.platform.application.AdminUpstreamConsumer;
...
public class PlatformOutboxConfig {
    ...
    InMemoryDurableEventLog inMemoryDurableEventLog(
            InProcessCoreEventBus bus,
            IdempotentConsumerSkeleton consumers,
            ObjectProvider<AdminUpstreamConsumer> adminConsumer) {
        ...
        log.addListener((stream, envelope) -> {
            if (!stream.endsWith(".dlq")) {
                ...
                boolean first = consumers.accept(envelope);
                if (first) {
                    AdminUpstreamConsumer admin = adminConsumer.getIfAvailable();
                    if (admin != null) {
                        admin.consume(envelope);
                    }
                }
            }
        });
